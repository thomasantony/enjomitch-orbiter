...
#ifndef MFDHUDDRAWER_HPP
#define MFDHUDDRAWER_HPP
...
capability of drawing on HUD from other context, in this case MFD.
You should only use the static methods of this class.
See the example in MultipleVesselsMFD package
*/
class MFDHUDDrawer : public
#ifdef ORB2006
VESSEL2
#else
VESSEL3
...
private:
	...
	private:
        ...
        friend class IMFDDrawsHUD;
        static void OnMFDConstruction( IMFDDrawsHUD * mfd );
        static void OnMFDDestruction();
        static bool IsVesselCompatible( VESSEL * v );

        static void TryHook( OBJHANDLE old_focus );
        static void TryUnHook( OBJHANDLE new_focus );

        static class IMFDDrawsHUD * mfdDrawsHUD;
};
}
}

#endif // MFDHUDDRAWER_HPP
