...
#ifndef FILEUTILSORBITER_H
#define FILEUTILSORBITER_H

#include <string>

class VESSEL;

class FileUtilsOrbiter
{
    public:
        FileUtilsOrbiter();
        virtual ~FileUtilsOrbiter();

        std::string GetMFDConfigDir() const;
        std::string GetMFDConfigFile() const;
        std::string GetVesselConfigDir( const VESSEL * v ) const;
        std::string GetVesselConfigFileName( const VESSEL * v, const std::string & suffix = "" ) const;

    protected:
    private:
};

#endif // FILEUTILSORBITER_H
