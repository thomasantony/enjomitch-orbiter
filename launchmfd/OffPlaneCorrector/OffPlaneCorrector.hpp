...
#ifndef OFFPLANECORRECTOR_HPP_INCLUDED
#define OFFPLANECORRECTOR_HPP_INCLUDED

class MFDDataLaunchMFD;

class OffPlaneCorrector
{
public:
    OffPlaneCorrector();
    double Calculate(char * tgt, MFDDataLaunchMFD * data );
    double GetCorrectionAngle() const;
    bool IsValid() const;

protected:
    virtual double CalculateClient( double timeToNode, MFDDataLaunchMFD * data ) = 0;
    bool IsAbovePlane( double timeToNode, MFDDataLaunchMFD * data );
    double GetCorrectionTimePositive( double tMeco, double timeToNode, MFDDataLaunchMFD * data, double limit );


	bool isValid;
	const double limit; // 45 degrees
private:
    double AdjustSilencer( double adjust, MFDDataLaunchMFD * data ) const;
