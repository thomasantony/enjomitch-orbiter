...
//                         All rights reserved
//
// FlightDirector.h - representation of required pitch and yaw on HUD
...
public:
	FlightDirector(void);
	~FlightDirector(void);
	void SetParams(double pitch, double azimuth);
	virtual void DrawHUD(int mode, const HUDPAINTSPEC *hps, MyDC hDC);
	void SetTailSitter(bool isTailSitter) {tailSitter = isTailSitter;};

// was private
	VECTOR3 GetTargetVector(VESSEL * vessel, double pitch, double azimuth);


private:
	double targetPitch;
	double targetAzimuth;
	bool tailSitter;	// Has no-main engine and uses hover thruster as the main engine (eg Apollo LM)
};
