...
#ifndef STAGINGCONTAINER_H
#define STAGINGCONTAINER_H
...
class StagingContainer : public EnjoLib::NonCopyable
{
    public:
        StagingContainer();
        virtual ~StagingContainer();
        const std::vector<Stage> & GetStages() const;
        double GetTMECORemaining( unsigned currentStage ) const;
        void AddStage( const Stage & stage );

		void ReadConfig(FILEHANDLE fh, FILEHANDLE fhVar);
        void SaveConfig(FILEHANDLE fhVar) const;

    protected:
    private:
        std::string GetTag(const std::string & tag, int i) const;
        std::vector<Stage> ReadFromFileHandle( FILEHANDLE fh ) const;

        std::vector<Stage> m_stagesRead;
        std::vector<Stage> m_stagesNew;
        const std::string m_tagStageTMECO;
        const std::string m_tagStageACCEL;
        const std::string m_tagStageISP;
};

#endif // STAGINGCONTAINER_H
