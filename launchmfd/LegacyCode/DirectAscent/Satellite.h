...
class Satellite : public Vessel
{
    public:
        Satellite();
        virtual ~Satellite();

        double GetTime( const Times & times );

        double delay;  // sattelite's delay on its path in seconds
        double angEqu; // angle between satellite and longitude 0* on the equator
        double orbitPeriod;     // sattelite's orbital period
