...
double Satellite::GetTime( const Times & times )
{
    while (delay > orbitPeriod)
        delay -= orbitPeriod;
    double stationTime = (times.total_t - delay);
    if ( stationTime < 0 ) stationTime += orbitPeriod;
    while ( stationTime > orbitPeriod && orbitPeriod > 0 )
        stationTime -= orbitPeriod;
