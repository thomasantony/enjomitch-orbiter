...
class PitchSchedule :
	public PitchGuidance
{
private:
	class AltitudePitchPair
	{
	public:
		double altitude;
		double pitch;
		int operator<(const AltitudePitchPair &rhs)	{return altitude < rhs.altitude;};
		bool operator!=(const AltitudePitchPair &rhs) {return altitude != rhs.altitude || pitch != rhs.pitch;}
		istream& operator<<(istream& in) {char ignore[256]; in >> altitude >> ignore >> pitch; return in;}
	};
...
public:
	...
	double GetTMECO(){return 0;};
	void SetApses(double a, double b){};
	bool IsValid(){return isValid;};

private:
	void ReadPitches(char *filename);
	bool isValid;

private:
	ifstream file;
	list<AltitudePitchPair> pitches;
};
