...
StagingDetector::StagingDetector(const VESSEL * v)
{
    shouldRecord = false;
    timeNoted = oapiGetSimTime();
    prevMass = v->GetMass();
}

StagingDetector::~StagingDetector()
{

}

bool StagingDetector::Update( const VESSEL * v )
{
    const double minPrecentageMassChange = 0.05;
    const int minTimeAfterStaging = 10;
    double currentMass = v->GetMass();

   // stageNum = 2;
    double precentageMassChange = fabs(currentMass - prevMass) / prevMass;
    if ( precentageMassChange > minPrecentageMassChange )
    {
        shouldRecord = true;
        timeNoted = oapiGetSimTime();
        savedPrecentageMassChange = precentageMassChange;
    }
	prevMass = currentMass;

    if ( shouldRecord && (oapiGetSimTime() - timeNoted > minTimeAfterStaging ) )
    {
        shouldRecord = false;
        return true;

//        std::ofstream f(m_stagesFileName.c_str(), std::ios::app);
//        if ( f.is_open() )
//        {
//            f << "Vessel '" << FileUtils().GetVesselConfigFileName( GetVessel() ) << "'\n";
//            f << "Stage number = " << stageNum++ << ", time noted = " << timeNoted << "\n";
//            f << "Mass = " << currentMass << ", mass change = " << savedPrecentageMassChange * 100 << "%\n";
//            VECTOR3 vThrust;
//            GetVessel()->GetThrustVector(vThrust);
//            f << "Measured total thrust = " << length(vThrust);
//            f << "\n(find matching thrusters below and add them to vessel's config)\n";
//            f << "ISP\t\tThrust\t\tLevel\n";
//            for ( unsigned i = 0; i < GetVessel()->GetThrusterCount(); ++i )
//            {
//                THRUSTER_HANDLE th = GetVessel()->GetThrusterHandleByIndex(i);
//                double isp = GetVessel()->GetThrusterIsp(th);
//                double level = GetVessel()->GetThrusterLevel(th);
//                double thrust = GetVessel()->GetThrusterMax(th);
//                if ( thrust * level > 15000 )
//                {
//                     f << isp << "\t" << thrust << "\t" << level << "\n";
//                }
//            }
