...
PEGDirectAscentStd::PEGDirectAscentStd( const VESSEL * v, const ShipVariables & vars )
: PEGDirectAscent( v, vars )
, cruiseEngineLevel(0.6)
{
}

PEGDirectAscentStd::~PEGDirectAscentStd()
{
}

Engine PEGDirectAscentStd::GetEngineCapabilities(const VESSEL * v) const
{
    ...
    if ( ! IsToTarget() )
    {
        Engine engEstimate = vc.EstimateMainThrustParm( v, cruiseEngineLevel );
        engCurrent = engEstimate;
    }
    return engCurrent;
}

void PEGDirectAscentStd::UpdateClient( MFDDataLaunchMFD * data )
{
    // Basic parameters
    ...
    double finalTrLVessel = GetFinalTrueLongitudeVessel( vessel, op );
    double deltaTrL = finalTrLVessel - tgt.TrL;
    deltaTrL = GeneralMath().GetInPIRange( deltaTrL );
    targetT = deltaTrL / (tgt.velRad);
    deltaT = targetT - GetTMECO();
}

double PEGDirectAscentStd::GetFinalTrueLongitudeVessel( const VESSEL * v, const ORBITPARAM & op)
{
    double finalTrL = op.TrL + GetDeltaTheta();
    finalTrL = GeneralMath().GetIn2PIRange( finalTrL );
    return finalTrL;
}

double PEGDirectAscentStd::GetDeltaT() const
{
    return deltaT;
}

double PEGDirectAscentStd::GetTargetT() const
{
    return targetT;
}

double PEGDirectAscentStd::GetCruiseEngineLevel() const
{
    return cruiseEngineLevel;
}
