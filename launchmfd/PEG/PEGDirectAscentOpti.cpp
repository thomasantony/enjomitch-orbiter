...
PEGDirectAscentOpti::~PEGDirectAscentOpti()
{
    //dtor
}

Engine PEGDirectAscentOpti::GetEngineCapabilities(const VESSEL * v) const
{
    VesselCapabilities vc;
    Engine engEstimate = vc.EstimateMainThrustParm( v, m_engineLevel );
    return engEstimate;
}

void PEGDirectAscentOpti::SetEngineLevel( double level )
{
    m_engineLevel = level;
}
