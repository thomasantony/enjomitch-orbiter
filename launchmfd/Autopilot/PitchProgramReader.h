...
#ifndef PITCHPROGRAMREADER_H
#define PITCHPROGRAMREADER_H
...
class PitchProgramReader
{
    public:
        PitchProgramReader( const VESSEL * v );
        virtual ~PitchProgramReader();
        double GetPitch( MFDDataLaunchMFD * data, double time ) const;
        bool HasProgram( const VESSEL * vessel ) const;

    protected:
    private:
        double LinearInterpolationProgramme( const std::vector< EnjoLib::Point > & programme, double x ) const;
        void ParseSections( const std::vector<std::vector<std::string> > & sections, bool timeBased);
        void ParseSection( const std::vector<std::string> & section, bool timeBased);
        std::string GetPlanetName( const VESSEL * vessel ) const;

        std::map<std::string, std::vector< EnjoLib::Point > > m_programmesAlt;
        std::map<std::string, std::vector< EnjoLib::Point > > m_programmesTime;
};

#endif // PITCHPROGRAMREADER_H
