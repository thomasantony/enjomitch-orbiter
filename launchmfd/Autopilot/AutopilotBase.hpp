...
public:
    AutopilotBase();
    virtual bool Guide( MFDDataLaunchMFD * data, double dt ) const;
    virtual std::string GetIdentifier() const = 0;

protected:

    virtual bool NeedPitch( MFDDataLaunchMFD * data ) const = 0;
    virtual bool NeedPullUp( MFDDataLaunchMFD * data ) const = 0;
    virtual bool NeedYaw( MFDDataLaunchMFD * data ) const = 0;
    virtual bool NeedMECO( MFDDataLaunchMFD * data ) const = 0;
    virtual bool TerminationRequested( MFDDataLaunchMFD * data ) const = 0;

private:
    bool NeedPitchMain( MFDDataLaunchMFD * data ) const;
    double UpdateBankUpsideDown( MFDDataLaunchMFD * data, double inputBank ) const;
    void SpaceAutopilot( MFDDataLaunchMFD * data, double dt, const EnjoLib::Point & targetVector2D, const VECTOR3 & accRatio ) const;
    void AtmoAutopilot( MFDDataLaunchMFD * data, double dt, const EnjoLib::Point & targetVector2D, const VECTOR3 & accRatio ) const;
    double CalculateRatioKdForAtmAP( const MFDDataLaunchMFD * data ) const;
};
