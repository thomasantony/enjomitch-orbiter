...
std::string Time::CurrentDateTime() const
{
    return CurrentDateTimeFormated("%Y-%m-%d %X");
}

std::string Time::CurrentDateTimeFormated(const std::string & format) const
{
    time_t     now = time(0);
    struct tm  tstruct;
    char       buf[256];
...
std::string Time::TimeStampToHuman(const time_t rawtime) const
{
    return TimeStampToHuman(rawtime, "%Y-%m-%d %X");
}

std::string Time::TimeStampToHuman(const time_t rawtime, const std::string & format) const
{
    struct tm * dt;
    char buffer [80];
