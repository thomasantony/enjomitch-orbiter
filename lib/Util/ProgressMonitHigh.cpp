...
void ProgressMonitHigh::PrintProgressBarTime(size_t i, size_t sz, const std::string & id, bool onPercentChange) const
{
    ostringstream oss;
    ...
    if (! onPercentChange || percentDone != m_prevPercentDone)
    {
        ...
        oss << id << ": i = " << i << " / " << sz << " = " << percentDone << "%" << " " << pm.GetProgressBar(i, sz)
