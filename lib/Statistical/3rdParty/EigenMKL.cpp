...
static UniquePtr<MKL_Complex16> ConvertMatrix( const Matrix & m )
{
    ...
    return UniquePtr<MKL_Complex16>(mat);
}
#endif
...
std::vector<EigenValueVector> EigenMKL::GetEigenValVecClient( const Matrix & m ) const
{
    ...
    const UniquePtr<MKL_Complex16> & matE = ConvertMatrix(m);
    MKL_INT n = m.GetNCols(), lda = m.GetNRows(), info;
    //MKL_INT n = m.GetNRows(), lda = m.GetNCols(), info;
    UniquePtr<double> w(new double[m.GetNCols()]);
