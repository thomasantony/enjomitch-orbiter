	...
	for (i=0;i<id;i++) {
		...
		for (j=0;j<id;j++) {
			...
			if (References[j].grf_handle==ref && References[j].handle!=ref) { // Anything orbiting this object
				...
				if (References[i].sys_index>255) return;
			}
		}
	}  	
	...
	for (i=0;i<count;i++) {
		...
		for (j=0;j<gcount;j++) {
			if (References[j].handle==ref) { 
				...
				if (References[j].sys_index>255) return;
			}
		}	
	}
}
...
{
	int i, gcount = oapiGetGbodyCount();
	for (i=0;i<gcount;i++) if (References[i].handle==x) return(References[i].system);
	return(NULL);	
}
...
{
	int i,gcount = oapiGetGbodyCount();
	for (i=0;i<gcount;i++) if (References[i].handle==x) return(References[i].sys_index);
	return(0);	
}
...
double ReferenceClass::GetSOI(OBJHANDLE obj)
{
	...
	if (r!=obj) return CalculateSOI(obj,r);
